...
- **AI 观点流** — 「🤖 AI 观点流」以信息流展示每条微博 + AI 解读（方向/情绪/板块/个股/关键词），顶部含 **AI 研判摘要**（多空/主线板块/分歧点），支持看多/看空/中性方向筛选
- **添加博主** — 输入 UID 或昵称；昵称可先「🔍 按昵称搜索」再点候选卡片添加
- **勾选博主** — 点击卡片多选，支持全选/清空
- **博主排名** — 「🏆 博主排名」展示算法 TOP10 与优选 TOP3，可一键「选入分析」
- **行情回测** — 「📊 行情回测准确率」用行情校验博主观点命中率，结果反哺排名
- **投资建议** — 「💡 生成投资建议」聚合博主观点，输出市场共识/热门板块/操作建议（支持时间范围、仅 TOP3）
...
用真实行情数据回测博主历史观点，把「准确率」维度从代理指标升级为**真实命中率**：

```bash
wft backtest                      # 真实行情（新浪接口），需博主观点含 6 位股票代码
wft backtest --demo               # 模拟行情演示
wft backtest --horizon 10         # 观点发布后 10 天检验涨跌
wft backtest --threshold 0.03     # 涨跌判定阈值 ±3%
```

判定逻辑：看多且区间涨幅 > 阈值 → 命中；看空且跌幅 > 阈值 → 命中。准确率写入数据库，
样本 ≥ 3 时自动替换 `wft rank` 的准确率维度（不足则回退置信度代理）。
...
| 维度 | 权重 | 说明 |
|------|------|------|
| 准确率 | 35% | 真实行情回测命中率（`wft backtest`，样本≥3）；无回测时回退置信度代理 |
